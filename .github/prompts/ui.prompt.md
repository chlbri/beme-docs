...
```
src/
├── features/               # Business features (START HERE)
│   └── [feature-name]/    # A specific feature
│       ├── front/         # Frontend code for the feature
│       │   ├── components/    # Feature-specific components
│       │   ├── signals/       # Feature-local state
│       │   └── utils/         # Feature frontend utilities
│       └── back/          # Feature backend code (optional)
│
└── globals/ui/            # Shared components (AFTER reuse)
    ├── cn/                # ⚠️ DO NOT MODIFY - Managed by shadcn-solid CLI
    │   ├── utils.ts       # cn() utility for CSS classes
    │   └── components/ui/ # Components generated by CLI (accordion, button, etc.)
    ...
    │   └── HeadLinks.tsx
    ├── helpers/           # Shared UI utility functions
    │   ├── fcc.ts        # Component helpers
    │   ├── seo.ts        # SEO helpers
...
2. **Migration to `globals/` only if reused**
   - If a component is used in 2+ features/routes → move to
     `globals/ui/molecules/` or `globals/ui/organisms/`
   - If a signal is used in 2+ features → move to `globals/ui/signals/`

3. **⚠️ NEVER modify `src/globals/ui/cn/components/ui/`**
...
### ⚠️ Base UI Component (primitive) - Use the CLI
...
2. **Create in the feature**
   ...
   ├── front/
   │   ├── components/
   │   │   └── MyComponent.tsx    ← Start here
   │   ├── signals/
   │   │   └── mySignal.ts         ← Local state
   │   └── utils/
   │       └── helpers.ts           ← Frontend utilities
   └── back/                        ← Backend code (optional)
   ```
...
- ✅ **ALWAYS** start in `src/features/[feature-name]/front/`
- ✅ Components in `front/components/`, signals in `front/signals/`
- ✅ Migrate to `globals/` **only** if reused
- ❌ **NEVER** manually create/modify in `cn/components/ui/`
